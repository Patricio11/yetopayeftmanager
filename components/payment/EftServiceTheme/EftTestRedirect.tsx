    ...
    const handleTestRedirect = async () => {
      try {
        ...
        if (test === 'true' && service === 'yetopay') {
          ...
          setError('Invalid test parameters. Use ?test=true&service=yetopay');
          setLoading(false);
        }
      } catch (err) {
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-gray-500">
            Expected URL format: /payment?test=true&service=yetopay&amount=100.00
          </p>
        </div>
      </div>
    );
